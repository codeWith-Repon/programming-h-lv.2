...
--  if same name of column is exist then you can use this shorten method

SELECT employee_name, department_name FROM employees
 JOIN departments USING(department_id)

-- Group By Department with Average Salary
SELECT department_name, round(AVG(salary)) as avarageSalary FROM employees
    JOIN departments USING(department_id) 
    GROUP BY department_name

-- Count Employees in Each Department
SELECT department_name, count(*) as total_student FROM employees
    JOIN departments USING(department_id)
    GROUP BY department_name
...
-- see count of order a user
SELECT customer_id, count(customer_id) FROM  orders
GROUP BY customer_id;

SELECT customer_id,
count(customer_id) as number_or_order,
